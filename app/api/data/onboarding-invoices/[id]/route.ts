...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const body = (await request.json()) as CreateOnboardingInvoiceInput
    await store.update(authResult.orgId, id, body)
    return NextResponse.json({ success: true })
  } catch (error) {
    const message =
      error instanceof Error ? error.message : 'Failed to update onboarding invoice'
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to delete onboarding invoice'
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
