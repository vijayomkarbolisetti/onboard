import { NextResponse } from 'next/server'
import { createOrgFirestoreStore } from '@/lib/orgFirestore'
import { isTeamAuthContext, requireModuleAccess } from '@/lib/team-auth'
import type { CreatePaidInvoiceInput, PaidInvoice } from '@/types'

const store = createOrgFirestoreStore<PaidInvoice>('paid_invoices')

type RouteContext = {
  params: Promise<{ id: string }>
}

export async function PATCH(request: Request, context: RouteContext) {
  const authResult = await requireModuleAccess('paid-invoices', 'write')
  if (!isTeamAuthContext(authResult)) {
    return authResult
  }

  const { id } = await context.params

  try {
    const body = (await request.json()) as CreatePaidInvoiceInput
    await store.update(authResult.orgId, id, body)
    return NextResponse.json({ success: true })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to update paid invoice'
    return NextResponse.json({ error: message }, { status: 500 })
  }
}

export async function DELETE(_request: Request, context: RouteContext) {
  const authResult = await requireModuleAccess('paid-invoices', 'write')
  if (!isTeamAuthContext(authResult)) {
    return authResult
  }

  const { id } = await context.params

  try {
    await store.remove(authResult.orgId, id)
    return NextResponse.json({ success: true })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to delete paid invoice'
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
