...
import { suggestOpenInvoiceFromFile } from '@/lib/openInvoiceExcel'
...
export function OpenInvoiceFormModal({
  ...
  const [form, setForm] = useState<OpenInvoiceFormState>(emptyForm)
  ...
  const applyAutofill = (suggestion: Partial<CreateOpenInvoiceInput>) => {
    const filled: string[] = []
    setForm((prev) => {
      const next = { ...prev }
      const apply = (key: keyof OpenInvoiceFormState, value?: string) => {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        uploaded.push(await uploadDocument(file, 'open-invoices'))
      }
      ...
      if (focusDoc && firstFile) {
        ...
        const suggestion = await suggestOpenInvoiceFromFile(firstFile)
